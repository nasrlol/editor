...
ENTRY_POINT(EntryPoint)
{
    if(LaneIndex() == 0)
    {
        ...
        b32 BuildEditorMetaprogram = true;
        ...
        if(BuildEditorMetaprogram)
        {
            ...
            if(Parse.errors.max_message_kind < MD_MessageKind_Error)
            {
                ...
                {                
                    ...
                    MD_Node *ShaderPreTable = MD_FirstNodeWithString(Root, MD_S8Lit("VertRectPreCode"), 0);
                    MD_Node *ShaderPostTable = MD_FirstNodeWithString(Root, MD_S8Lit("VertRectPostCode"), 0);
                    
                    MD_S8ListPush(GlobalMDArena, &ShaderStream, ShaderPreTable->first_child->string);
                    ...
                    MD_S8ListPush(GlobalMDArena, &ShaderStream, ShaderPostTable->first_child->string);
        ...
        if(BuildEditor)
        {
            ...
            if(Windows)
            {
                if(!OS_FileExists(LibsFileName))
                {
                    Str8ArrayPushCount(EditorFlags)
                    {                        
                        Str8ArrayAppendMultipleTo(EditorFlags, S8("-DEDITOR_SLOW_COMPILE=1"
                                                                  "-Fmeditor_app.map"),
                                                  S8("-Fo"),
                                                  S8FromCString(LibsFileName),
                                                  S8("-c /TP -LD "));
