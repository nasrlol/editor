...
AppendChar(app_state *App, rune Codepoint)
{
  ...
  App->Text[App->TextCount] = (u8)Codepoint;
  App->TextCount += 1;
  App->CursorPos += 1;
...
InitAtlas(arena *Arena, app_font *Font, font_atlas *Atlas, f32 HeightPx)
{
  ...
  stbtt_pack_context ctx;
  {
    ...
    stbtt_PackFontRange(&ctx, Font->Info.data, 0, HeightPx, Atlas->FirstCodepoint, Atlas->CodepointsCount, Atlas->PackedChars);
    stbtt_PackEnd(&ctx);
  }

    for
      EachIndex(Idx, Atlas->CodepointsCount)
      {
        float UnusedX, UnusedY;
        stbtt_GetPackedQuad(Atlas->PackedChars, Atlas->Width, Atlas->Height, (u32)Idx, &UnusedX, &UnusedY, &Atlas->AlignedQuads[Idx], 0);
      }
}

void
DrawChar(font_atlas *Atlas, v2 *Positions, v2 *TexCoords, s32 *VerticesCount, v2 Pos, rune Codepoint)
{
...
UPDATE_AND_RENDER(UpdateAndRender)
{
  ...
  // Text input
    for
      EachIndex(Idx, Input->Text.Count)
      {
    ...
    else if(Input->PlatformIsRecording)
      WindowBorderColor = Color_Red;
    else if(Input->PlatformIsPlaying)
      WindowBorderColor = Color_Green;
    else if(Input->PlatformWindowIsFocused)
      WindowBorderColor = Color_Foreground;
    else
      WindowBorderColor = Color_Background;
    ...
    {
      glBindVertexArray(VAOs[0]);
      RectShader = gl_ProgramFromShaders(FrameArena, Memory->ExeDirPath, S8("../source/shaders/rect_vert.glsl"), S8("../source/shaders/rect_frag.glsl"));
    ...
    // Render text
    {
      ...
      TextShader = gl_ProgramFromShaders(FrameArena, Memory->ExeDirPath, S8("../source/shaders/text_vert.glsl"), S8("../source/shaders/text_frag.glsl"));
