...
//- Rendering
internal void
DrawCharacter(app_offscreen_buffer *Buffer, u8 *FontBitmap, int FontWidth, int FontHeight, int XOffset, int YOffset, u32 Color)
{
    s32 MinX = 0;
    s32 MinY = 0;
    s32 MaxX = FontWidth;
...
internal void
DrawText(arena *Arena, app_offscreen_buffer *Buffer, app_font *Font, f32 HeightPixels, v2 Offset, b32 IsUTF8, u32 Color, str8 Text)
{
    if(Font->Initialized)
    {
        ...
        for
            EachIndex(Idx, Text.Size)
            {
                ...
                s32 X0, Y0, X1, Y1;
                u8 *FontBitmap = 0;
                stbtt_GetCodepointBitmapBox(&Font->Info, CharAt, FontScale, FontScale, &X0, &Y0, &X1, &Y1);
                FontWidth  = (X1 - X0);
                FontHeight = (Y1 - Y0);
                FontBitmap = PushArray(Arena, u8, (FontWidth * FontHeight));
                stbtt_MakeCodepointBitmap(&Font->Info, FontBitmap, FontWidth, FontHeight, FontWidth, FontScale, FontScale, CharAt);
...
internal void
DrawTextInBox(arena *Arena, app_offscreen_buffer *Buffer, app_font *Font, str8 Text, f32 HeightPx, u32 Color, v2 BoxMin, v2 BoxMax, b32 Centered)
{
    ...
    u32 Start = 0;
    for
        EachIndex(WrapIndex, WrapPositionsCount)
        {
            ...
            if(TextOffset.Y - FontScale * ((f32)Font->Descent) < BoxMax.Y)
            {
                ...
                DrawText(Arena, Buffer, Font, HeightPx, TextOffset, false, Color, S8FromTo(Text, Start, Position));
            }
