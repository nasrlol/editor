...
//- Helpers
static int
XCtxErrorHandler(Display *dpy, XErrorEvent *ev)
{
    XCtxError = true;
    return 0;
}
...
internal void
LinuxSetSizeHint(Display *DisplayHandle, Window WindowHandle, int MinWidth, int MinHeight, int MaxWidth, int MaxHeight)
{
    XSizeHints Hints = {};
...
LinuxShowCursor(linux_x11_context *Linux, b32 Entered)
{
    if(Entered)
    {
        ...
        Cursor InvisibleCursor = XCreatePixmapCursor(Linux->DisplayHandle, BitmapNoData, BitmapNoData, &black, &black, 0, 0);
...
P_ContextInit(arena *Arena, app_offscreen_buffer *Buffer, b32 *Running)
{
    ...
    if(DisplayHandle)
    {
        ...
        if(OpenGLMode)
        {
            ...
            {
            GLX_X_RENDERABLE, True, GLX_DRAWABLE_TYPE, GLX_WINDOW_BIT, GLX_RENDER_TYPE, GLX_RGBA_BIT, GLX_X_VISUAL_TYPE, GLX_TRUE_COLOR, GLX_RED_SIZE, 8, GLX_GREEN_SIZE, 8, GLX_BLUE_SIZE, 8, GLX_ALPHA_SIZE, 8, GLX_DEPTH_SIZE, 24, GLX_STENCIL_SIZE, 8, GLX_DOUBLEBUFFER, True,
            ...
            if(IsCorrectVersion)
            {
                ...
                if(FBConfigs)
                {
                    ...
                    for
                        EachIndexType(s32, FBIdx, FBConfigsCount)
                        {
        ...
        if(Found)
        {
            ...
            Window WindowHandle = XCreateWindow(DisplayHandle, RootWindow, X, Y, Buffer->Width, Buffer->Height, 0, WindowVisualInfo.depth, InputOutput, WindowVisualInfo.visual, WindowAttributeMask, &WindowAttributes);
            ...
            if(WindowHandle)
            {
                // Window attributes and hints
                {
                    ...
                    // Disable window decorations
                    {
                        ...
                        XChangeProperty(DisplayHandle, WindowHandle, MotifWMHintsAtom, MotifWMHintsAtom, 32, PropModeReplace, (unsigned char *)&hints, sizeof(hints) / sizeof(long));
                    }
                ...
                XRet                      = XSetWMProtocols(DisplayHandle, WindowHandle, &Context->WM_DELETE_WINDOW, 1);
                Assert(XRet);
                ...
                case KeyRelease:
                {
                    ...
                    // Text input
                    {
                        ...
                        if(IsDown && !FilteredEvent)
                        {
                            ...
                            if(Shift)
                                Button->Modifiers |= PlatformKeyModifier_Shift;
                            if(Control)
                                Button->Modifiers |= PlatformKeyModifier_Control;
                            if(Alt)
                                Button->Modifiers |= PlatformKeyModifier_Alt;

                            s32 BytesLookepdUp = Xutf8LookupString(Linux->InputContext, &WindowEvent.xkey, (char *)&LookupBuffer, ArrayCount(LookupBuffer), 0, &LookupStatus);
                            ...
                            if(!KeyHasLookup || Button->IsSymbol)
                            {
                                ...
                                if(0)
                                {
                                    ...
                                    Button->Symbol = PlatformKey_Escape;
                                else if(Symbol == XK_Tab)
                                    Button->Symbol = PlatformKey_Tab;
                                else if(Symbol == XK_BackSpace)
                                    Button->Symbol = PlatformKey_BackSpace;
                    ...
                    if(0)
                    {
                        ...
                        ProcessKeyPress(&Input->MoveLeft, IsDown);
                    else if(Symbol == XK_s)
                        ProcessKeyPress(&Input->MoveRight, IsDown);
                    else if(Symbol == XK_w)
                        ProcessKeyPress(&Input->MoveUp, IsDown);
                    else if(Symbol == XK_r)
                        ProcessKeyPress(&Input->MoveDown, IsDown);
                }
                break;
