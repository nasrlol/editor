...
UI_CalculatePositionsAndDrawBoxes(ui_box *Box)
{
       ...
       Dest.Min.Y < Dest.Max.Y)
    {
        ...
        if(Box->Flags & UI_BoxFlag_DrawDisplayString)
        {
            ...
            for
                EachIndex(Idx, Box->DisplayString.Size)
                {
...
UI_DebugPrintBoxes(ui_box *Box)
{
    ...
    UI_DebugIndentation,
    "",
    S8Arg(Box->DisplayString),
    UI_DebugIndentation,
    "",
    Box->SemanticSize[0].Kind,
    Box->SemanticSize[1].Kind,
    UI_DebugIndentation,
    "",
    Box->FixedPosition.X,
    Box->FixedPosition.Y,
