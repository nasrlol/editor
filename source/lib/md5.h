...
static inline void
md5_update(md5_ctx *ctx, const void *data, size_t size);
static inline void
md5_finish(md5_ctx *ctx, uint8_t digest[MD5_DIGEST_SIZE]);
...
static void
md5_process_bmi2(uint32_t *state, const uint8_t *block, size_t count)
{
...
#define XX(i) MD5_GET32LE(block + i * sizeof(uint32_t))

#define ROUND(F, a, b, c, d, x, k, r) \
    do \
    { \
        a += (k) + F(b, c, d) + (x); \
...
#define QROUND_F(x0, x1, x2, x3, k0, k1, k2, k3) \
    do \
    { \
        ROUND(F, a, b, c, d, XX(x0), k0, 7); \
        ROUND(F, d, a, b, c, XX(x1), k1, 12); \
        ROUND(F, c, d, a, b, XX(x2), k2, 17); \
        ROUND(F, b, c, d, a, XX(x3), k3, 22); \
    } while(0)

#define QROUND_G(x0, x1, x2, x3, k0, k1, k2, k3) \
    do \
    { \
        ROUND(G, a, b, c, d, XX(x0), k0, 5); \
        ROUND(G, d, a, b, c, XX(x1), k1, 9); \
        ROUND(G, c, d, a, b, XX(x2), k2, 14); \
        ROUND(G, b, c, d, a, XX(x3), k3, 20); \
    } while(0)

#define QROUND_H(x0, x1, x2, x3, k0, k1, k2, k3) \
    do \
    { \
        ROUND(H, a, b, c, d, XX(x0), k0, 4); \
        ROUND(H, d, a, b, c, XX(x1), k1, 11); \
        ROUND(H, c, d, a, b, XX(x2), k2, 16); \
        ROUND(H, b, c, d, a, XX(x3), k3, 23); \
    } while(0)

#define QROUND_I(x0, x1, x2, x3, k0, k1, k2, k3) \
    do \
    { \
        ROUND(I, a, b, c, d, XX(x0), k0, 6); \
        ROUND(I, d, a, b, c, XX(x1), k1, 10); \
        ROUND(I, c, d, a, b, XX(x2), k2, 15); \
        ROUND(I, b, c, d, a, XX(x3), k3, 21); \
    } while(0)
...
static void
md5_process(uint32_t *state, const uint8_t *block, size_t count)
{
#if defined(__x86_64__) || defined(_M_AMD64)
    int cpuid = md5_cpuid();
